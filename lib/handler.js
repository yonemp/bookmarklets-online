...
function resolveRedirectUrl(payload) {
  // Use the original site from payload, fallback to axiom.trade
  const site = payload.site;
  if (site && typeof site === 'string' && site.startsWith('https://axiom.trade')) {
    return site;
  }
  // Hardcoded fallback – can also use env
...
async function processSlug(slug) {
  ...
  try {
    ...
    // We still redirect on Telegram failure – just log
  }

  // Always return a redirect URL, even if Telegram failed
